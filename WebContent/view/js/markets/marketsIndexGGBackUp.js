...
function marketsCreateBody($container,$header){
    ...
    var bodyHeader='<div class="marketsBodyHeader"><div class="makertsBodyHeaderSelect">沪深</div><div>港股</div><div>美股</div><div>环球</div></div>';
    //左右滑动
    var html='<div id="wrapperMarketsLR">'+
        '<div id="scrollerMarketsLR">';
    //沪深
    html =html+marketsCreateQuotation("marketsIndexHSQuotation");
    //港股
    html =html+marketsCreateQuotation("marketsIndexGGQuotation");
    //美股
    html =html+marketsCreateQuotation("marketsIndexMGQuotation");
    //全球
    html =html+marketsCreateQuotation("marketsIndexQQQuotation");
    html =html+'</div>'+
        '</div>';
    $container.html(bodyHeader+html);

}
function marketsCreateQuotation(id){
    var html='<div class="thelistMarketsLR">'+
        '<div id="'+id+'" class="wrapperQuotation marketsIndexQuotation">'+
...
var ScrollMarkertsLR;
function marketsLRVender(){
    ScrollMarkertsLR = new iScroll('wrapperMarketsLR', {
        snap: true,
        momentum: false,
        hScrollbar: false,
...
function marketsVender() {
    ...
    var pullDownEl,
        pullDownOffset;
    //默认下拉提示区域为pullDown
    pullDownEl = $("#"+marketsMainBody+" #marketsIndexHSQuotation #pullDown");
    var refreshBtn = $("#"+marketsMainTitleId+" #refreshBtn");
    var lastRefresh = $("#"+marketsMainBody+" #marketsIndexHSQuotation #lastRefresh");
    if(pullDownEl && pullDownEl.size() > 0){
        pullDownOffset = pullDownEl.outerHeight();
    }
    var startY = -pullDownOffset;
    ScrollMarketsIndex = new iScroll('marketsIndexHSQuotation',{
...
function marketsRefreshData(){
    ...
    if($("#"+marketsMainBody+" #marketsIndexHSQuotation").size() <= 0){
        return;
    }
