...
public class PageManagerNew extends PageServer{
	private HashMap<Long, CandlesDto> hashmap = null;
	private static PageManagerNew singleInstance = null;
	private List<BaseDto> list = null;
	private Iterator<BaseDto> it = null;
	private Entry<Integer, BaseDto> entry = null;
	private Iterator<Entry<Integer, BaseDto>> iter = null;

	public static PageManagerNew getSingleInstance(){
		if (singleInstance == null) {
			synchronized (PageManagerNew.class) {
				if (singleInstance == null) {
					singleInstance = new PageManagerNew();
				}
			}
		}
	 ...
	 */
	final private PageDto getPageDto(int period, int page) throws Exception {
		if(this.isCurrPage(period, page) && page != this.page_no){
			return this.getCurrPageDto(period);
		}else if(page == this.page_no){
			return null;
		}
		return PageDataFutrueTask.getSingleInstance(period, 1).getCurrentPageContent(page);
	}
	...
	final public CandlesDto getCandlesDtoByTime(int period, long time){
		...
		if(null != this.pdto){
			...
			//}catch (Exception e){
			//	System.out.println("this.hashmap is null : " + time);
			//}
		}
		this.hashmap = null;
		return null;
